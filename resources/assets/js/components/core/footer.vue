<template>
    <footer class="footer-section">
        <div class="head-footer-wrap-bigScreen">
            <div class="head-footer-wrap">
                <div class="site-naw-wrap">
                    <ul class="head-footer-menu">
                        <li>{{translate.company}}</li>
                        <li>{{translate.tool}}</li>
                        <li>{{translate.contacts}}</li>
                        <li>{{translate.address}}</li>
                    </ul>
                </div>
            </div>
            <div class="main-footer-wrap">
                <div class="all-items-menu-wrap">
                    <div class="single-menu-item">
                        <ul class="list-company-info">
                            <li><a href="https://agroyard.ua/news">{{translate.news}}</a></li>
                            <li><a href="https://agroyard.ua/about">{{translate.about}}</a></li>
                            <li><a href="https://agroyard.ua/callback">{{translate.feedback}}</a></li>
                        </ul>
                    </div>
                    <div class="single-menu-item">
                        <ul class="list-company-info">
                            <li><a href="https://agroyard.ua/auctions/all">{{translate.sell_crop}}</a></li>
                            <li><a href="https://agroyard.ua/classifieds/gsm?id=1">{{translate.adv}}</a></li>
                            <li><a href="https://agroyard.ua/auctions/all">{{translate.auction}}</a></li>
                            <li><a href="/sitemap.xml">Карта сайту</a></li>
                            <!--<li><a href="#">{{translate.tenders}}</a></li>-->
                        </ul>
                    </div>
                    <div class="single-menu-item">
                        <ul class="list-company-info">
                            <li><a href="tel:0800802141">(0800) 80 21 41</a></li>
                            <li><a href="tel:0501992141">(050) 199 21 41</a></li>
                            <li><a href="tel:0961992141">(067) 199 21 41</a></li>
                            <li><a href="tel:0931992141">(093) 199 21 41</a></li>
                            <li><a href="mailto:info@agroyard.com.ua">info@agroyard.ua</a></li>
                        </ul>
                    </div>
                    <div class="single-menu-item">
                        <ul class="list-company-info">
                            <li>{{translate.detail_address_1}}</li>
                            <li>{{translate.detail_address_2}}</li>
                        </ul>
                        <a href="https://www.instagram.com/agroyard/">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="37px" height="37px" viewBox="0 0 438.536 438.536" style="enable-background:new 0 0 438.536 438.536;" xml:space="preserve">
<g>
	<path d="M421.981,16.562C410.941,5.519,397.711,0,382.298,0H56.248C40.83,0,27.604,5.521,16.561,16.562   C5.52,27.6,0.001,40.828,0.001,56.243V382.29c0,15.413,5.518,28.644,16.56,39.683c11.043,11.04,24.272,16.563,39.687,16.563   h326.046c15.41,0,28.644-5.523,39.684-16.563c11.043-11.039,16.557-24.27,16.557-39.683V56.243   C438.534,40.825,433.021,27.604,421.981,16.562z M157.462,158.025c17.224-16.652,37.924-24.982,62.097-24.982   c24.36,0,45.153,8.33,62.381,24.982c17.228,16.655,25.837,36.785,25.837,60.386c0,23.598-8.609,43.729-25.837,60.379   c-17.228,16.659-38.014,24.988-62.381,24.988c-24.172,0-44.87-8.336-62.097-24.988c-17.228-16.652-25.841-36.781-25.841-60.379   C131.621,194.81,140.234,174.681,157.462,158.025z M388.865,370.589c0,4.945-1.718,9.083-5.141,12.416   c-3.433,3.33-7.519,4.996-12.282,4.996h-305.2c-4.948,0-9.091-1.666-12.419-4.996c-3.333-3.326-4.998-7.471-4.998-12.416V185.575   H89.08c-3.805,11.993-5.708,24.462-5.708,37.402c0,36.553,13.322,67.715,39.969,93.511c26.65,25.786,58.721,38.685,96.217,38.685   c24.744,0,47.583-5.903,68.527-17.703c20.937-11.807,37.486-27.839,49.676-48.112c12.183-20.272,18.274-42.4,18.274-66.38   c0-12.94-1.91-25.406-5.715-37.402h38.547v185.014H388.865z M388.865,115.626c0,5.52-1.903,10.184-5.716,13.99   c-3.805,3.809-8.466,5.711-13.989,5.711h-49.676c-5.517,0-10.185-1.903-13.99-5.711c-3.806-3.806-5.708-8.47-5.708-13.99V68.522   c0-5.33,1.902-9.945,5.708-13.848c3.806-3.901,8.474-5.854,13.99-5.854h49.676c5.523,0,10.185,1.952,13.989,5.854   c3.812,3.903,5.716,8.518,5.716,13.848V115.626z" fill="#FFFFFF"/>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
</svg>
                        </a>
                        <a href="https://facebook.com/agroyard/"><img src="/img/footer/fb.png" alt="fb" style="width:37px; height: 37px"></a>
                    </div>
                    <div class="qr-wrap">
                    </div>
                </div>
            </div>
            <div class="partners">
                <div class="info-partners">
                    <p>При підтримці Швейцарсько-української програми EGAP проект «АгроЯрд Маркет» працює для Вас</p>
                    <a class="logo_egap" href="http://egap.in.ua/" title="Перейти на сайт EGAP!">
                        <img src="/img/footer/egap.png" title="Натисність для переходу на сайт EGAP!" alt="Логотип сайту EGAP" />
                    </a>
                </div>
            </div>
            <div class="bottom-footer-detail">
                <div class="all-det-wrap">
                    <div class="left">
                        <p>&#169; АгроЯрд 2018 г. {{translate.all_right}}</p>
                    </div>
                    <div class="right">
                        <p>{{translate.site_create}}</p>
                    </div>
                </div>
            </div>
        </div>
        <!-- footer-mobile -->
        <div class="head-footer-wrap-mobile">
                <div class="footer-content">
                    <div class="main-footer-block">
                        <div class="site-naw-wrap-left">
                            <ul class="head-footer-menu">
                                <li class ="general-footer-title">{{translate.company}}</li>
                                <li><a href="https://agroyard.ua/news">{{translate.news}}</a></li>
                                <li><a href="https://agroyard.ua/about">{{translate.about}}</a></li>
                                <li><a href="https://agroyard.ua/callback">{{translate.feedback}}</a></li>
                                <li><router-link :to="{name: 'classifieds', params: {slug: 'gsm'}, query: {id: 1}}">{{translate.tenders}}</router-link></li>
                                <li class ="general-footer-title">{{translate.contacts}}</li>
                                <li class="telephone"><a href="tel:0961992141">(067) 199 21 41</a></li>
                                <li class="telephone"><a href="tel:0931992141">(093) 199 21 41</a></li>
                                <li><a href="mailto:info@agroyard.ua">info@agroyard.ua</a></li>
                            </ul>
                        </div>
                        <div class="site-naw-wrap-right">
                            <ul class="head-footer-menu">
                                <li class ="general-footer-title">{{translate.tool}}</li>
                                <li><a href="https://agroyard.ua/account/auction-create">{{translate.sell_crop}}</a></li>
                                <li><a href="https://agroyard.ua/classifieds/gsm?id=1">{{translate.adv}}</a></li>
                                <li><a href="https://agroyard.ua/auctions/all">{{translate.auction}}</a></li>
                                <li class="telephone right"><a href="tel:0800802141">(0800) 80 21 41</a></li>
                                <li class="telephone"><a href="tel:0501992141">(050) 199 21 41</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class ="location-wrap">
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 487.724 487.724" style="enable-background:new 0 0 487.724 487.724;" xml:space="preserve">
    <g>
        <g>
            <path d="M236.925,0.124c-96.9,3.4-177.4,79-186.7,175.5c-1.9,19.3-0.8,38,2.6,55.9l0,0c0,0,0.3,2.1,1.3,6.1    c3,13.4,7.5,26.4,13.1,38.6c19.5,46.2,64.6,123.5,165.8,207.6c6.2,5.2,15.3,5.2,21.6,0c101.2-84,146.3-161.3,165.9-207.7    c5.7-12.2,10.1-25.1,13.1-38.6c0.9-3.9,1.3-6.1,1.3-6.1l0,0c2.3-12,3.5-24.3,3.5-36.9C438.425,84.724,347.525-3.776,236.925,0.124    z M243.825,291.324c-52.2,0-94.5-42.3-94.5-94.5s42.3-94.5,94.5-94.5s94.5,42.3,94.5,94.5S296.025,291.324,243.825,291.324z"/>
        </g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    <g>
    </g>
    </svg>
                        <ul class="list-company-info">
                            <li>{{translate.detail_address_1}}</li>
                            <li>{{translate.detail_address_2}}</li>
                        </ul>
                    </div>
                    <div class="socials">
                        <h3>Соцсети</h3>
                        <div class="soc-ico">
                            <a href="https://www.instagram.com/agroyard/">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="37px" height="37px" viewBox="0 0 438.536 438.536" style="enable-background:new 0 0 438.536 438.536;" xml:space="preserve">
<g>
	<path d="M421.981,16.562C410.941,5.519,397.711,0,382.298,0H56.248C40.83,0,27.604,5.521,16.561,16.562   C5.52,27.6,0.001,40.828,0.001,56.243V382.29c0,15.413,5.518,28.644,16.56,39.683c11.043,11.04,24.272,16.563,39.687,16.563   h326.046c15.41,0,28.644-5.523,39.684-16.563c11.043-11.039,16.557-24.27,16.557-39.683V56.243   C438.534,40.825,433.021,27.604,421.981,16.562z M157.462,158.025c17.224-16.652,37.924-24.982,62.097-24.982   c24.36,0,45.153,8.33,62.381,24.982c17.228,16.655,25.837,36.785,25.837,60.386c0,23.598-8.609,43.729-25.837,60.379   c-17.228,16.659-38.014,24.988-62.381,24.988c-24.172,0-44.87-8.336-62.097-24.988c-17.228-16.652-25.841-36.781-25.841-60.379   C131.621,194.81,140.234,174.681,157.462,158.025z M388.865,370.589c0,4.945-1.718,9.083-5.141,12.416   c-3.433,3.33-7.519,4.996-12.282,4.996h-305.2c-4.948,0-9.091-1.666-12.419-4.996c-3.333-3.326-4.998-7.471-4.998-12.416V185.575   H89.08c-3.805,11.993-5.708,24.462-5.708,37.402c0,36.553,13.322,67.715,39.969,93.511c26.65,25.786,58.721,38.685,96.217,38.685   c24.744,0,47.583-5.903,68.527-17.703c20.937-11.807,37.486-27.839,49.676-48.112c12.183-20.272,18.274-42.4,18.274-66.38   c0-12.94-1.91-25.406-5.715-37.402h38.547v185.014H388.865z M388.865,115.626c0,5.52-1.903,10.184-5.716,13.99   c-3.805,3.809-8.466,5.711-13.989,5.711h-49.676c-5.517,0-10.185-1.903-13.99-5.711c-3.806-3.806-5.708-8.47-5.708-13.99V68.522   c0-5.33,1.902-9.945,5.708-13.848c3.806-3.901,8.474-5.854,13.99-5.854h49.676c5.523,0,10.185,1.952,13.989,5.854   c3.812,3.903,5.716,8.518,5.716,13.848V115.626z" fill="#FFFFFF"/>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
<g>
</g>
</svg>
                            </a>
                            <a href="https://www.facebook.com/agroyard/">
                            <img class="img-fb-social-network" src="/img/footer/fb.png" alt="facebook" style="width: 37px; height: 37px;">
                            </a>
                        </div>
                        <div class="qr-wrap">
                        </div>
                    </div>
                    <div class="partners">
                        <div class="info-partners">
                            <p>При підтримці Швейцарсько-української програми EGAP проект «АгроЯрд Маркет» працює для Вас</p>
                            <a class="logo_egap" href="http://egap.in.ua/" title="Перейти на сайт EGAP!">
                                <img src="/img/footer/egap.png" title="Натисність для переходу на сайт EGAP!" alt="Логотип сайту EGAP" />
                            </a>
                        </div>
                    </div>
                    <div class="bottom-footer-detail">
                        <p>&#169; АгроЯрд 2018 г. {{translate.all_right}}</p>
                        <p>{{translate.site_create}}</p>
                    </div>
                </div>
        </div>
    </footer>
</template>

<script type="text/babel">
    export default {
        data(){
            return {}
        },
        props: [
            'translate'
        ]
    }
</script>