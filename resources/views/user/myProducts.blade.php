    ...
    @section('center_content')
        ...
        <div class="row">
           {{--<div class="col-lg-11">--}}
                ...
                {{--<li><a href="{{ route('users.products') }}">{{ trans('globals.active') }}</a></li>--}}
                {{--<li><a href="{{ route('users.products').'?inactives=1' }}">{{ trans('globals.inactive') }}</a></li>--}}
                {{--<li><a href="{{ route('users.products').'?low_stock=1' }}">{{ trans('product.inputs_view.low_stock') }}</a></li>--}}
                {{--</ul>--}}
                {{--</div>--}}
            {{--</div>--}}
                ...
                <div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                    <div class="thumbnail">
                        <div style="height: 18em;">
                            @if ($product->status == 1)
                                <span class="label label-success">{{ trans('globals.available') }}</span>
                            @endif
                            @if ($product->status == 0)
                                <span class="label label-warning">{{ trans('globals.unavailable') }}</span>
                            @endif
                            <a href="{{ route('products.index') }}/{{ $product->id }}">
                                @if (isset(\App\Models\UserProduct::images($product)[0]))
                                    <img class="thumbnail"
                                         src="{{ \App\Models\UserProduct::images($product)[0] }}?w=100" alt="{{ $product->mainProduct->name }}"
                                         width="100" height="100">
                                @else
                                    <img class="thumbnail" src="/img/no-image.jpg" alt="{{ $product->mainProduct->name }}" width="100" height="100">
                                @endif
                            ...
                            <p>
                                <strong>{{ Utility::showPrice($product->getPrice()) }}</strong>
                            </p>
                            <p>
                                <a class="btn-xs btn-primary" href="{{ route('products.index') }}/{{ $product->id }}/edit" role="button">{{ trans('globals.edit') }}</a>
                                <a class="btn-xs btn-default" href="{{ url('storeproduct') }}/{{ $product->id }}" role="button">{{ trans('product.globals.view_details') }} &raquo;</a>
                            </p>
                        </div>
                    </div>
                </div><!-- /.col-lg-4 -->
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="alert alert-warning">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            {{ trans('globals.message_no_products') }}.
                        </div>
                    </div>
                    <div class="row">&nbsp;</div>
                </div>
