...
class Menu
{
    ...
    public static function summary($returnArray = false)
    {
        ...
        if (auth()->check() && auth()->user()->hasRole(['seller'])) { //will move to the foundation panel
            $menu = array_merge($menu, [
                ['route' => route('users.products.list'), 'text' => trans('user.your_products'), 'icon' => 'glyphicon glyphicon-briefcase'],
    ...
    public static function top($returnArray = false)
    {
        if (\Auth::guest()) { // invidados
            ...
            $menu = [
                ['route' => '/dashboard', 'text' => trans('user.wpanel'), 'icon' => 'glyphicon glyphicon-cog'],
//                ['route' => '/wpanel', 'text' => trans('user.wpanel'), 'icon' => 'glyphicon glyphicon-cog'],
            ];
        } else {  // logeado
